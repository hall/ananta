= 9p intro: introduction to the file protocol 9P
...
An server is an agent that provides one or more hier‐
archical file systems — file trees — that may be accessed  by
processes.  A server responds to requests by clients
...
Another  possibility  for  a server is to synthesize files on
demand, perhaps  based  on  information  on  data  structures
inside the kernel; the prog(3) kernel device is a part of the
Inferno kernel that does this.  User programs can also act as
servers.   One  easy way is to serve a set of files using the
sys-file2chan(2) interface.  More complex Limbo file  service
applications  can use styx(2) to handle the protocol messages
directly, or use styxservers(2)  to  provide  a  higher-level
framework for file serving.
