...
Applications are written in link:{docpath}/limbo[limbo] and compiled for link:{docpath}/dis[dis] to run anywhere on a network in a portable operating system.

The design has three principles:

 .  All resources are named and accessed as files.
 .  One protocol (link:{docpath}/9p[9p]) allows uniform access to these resources.
